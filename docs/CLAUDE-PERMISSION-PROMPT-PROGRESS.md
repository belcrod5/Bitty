...
- 2026-08-23 実機検証で発見したハマりどころ(2件目): OAuth失効中にresumeターンが1.5秒で「Claude emitted system/init twice」(protocol_error)となる不具合。直前14分前の同一セッションresumeは成功しており、失敗直後にCLIの「Login expired」を別途確認した。原因は、認証が失効したCLIが内部再試行としてsystem/initを同一turn内で2回emitすること(実機で確認)。既存実装は`state.initialized`が真なら理由を問わず即`protocol_error`で落としていたため、CLI自身のerror result/stderrまで到達できず、`isClaudeAuthFailure`によるログイン案内メッセージへの分類が効かなかった(ユーザーには意味の薄いprotocol_errorだけが見えていた)。対策: system/initの2回目以降はsession idが変わっていなければ無視して継続する(冪等化。`provider.event(nativeType:"system/init_repeated")`で診断用に記録するのみ)。session idが変わっていた場合のみ`protocol_error`のまま(「Claude changed the session ID」)。tool.startedの冪等化と同型のパターンで、CLIの非決定的な再送に対してrunner側は「同一キーなら無視」で応じる以外に取れる対策がない
