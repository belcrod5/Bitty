    ...
    if (message.op === "agent.hello") {
      void service.getStatuses().then((backends) => sendEnvelope(ws, {
        ...
        payload: {
          ...
          operations: [
            "turn.start", "voice.open", "voice.status", "voice.settings", "voice.settings.update", "voice.memory.clear", "voice.messages.clear", "voice.approval.decision", "turn.interrupt", "action.claim", "action.respond", "events.resume", "events.detach", "session.handoff",
