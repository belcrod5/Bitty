...
async function listLlmSessionMessages(rawSessionId, opts = {}) {
  ...
  return {
    ...
    cwd: String(messagesResult?.workingDirectory || cliEntry.cwd || ""),
    isSubagent: messagesResult?.isSubagent === true,
...
async function fetchGitChangedFilesSnapshot(rawDirectory) {
  ...
  const [branchStatus, branches, stagedFiles, unstagedDiffFiles, untrackedFiles] = await Promise.all([
    fetchGitBranchStatus({ cwd, runCommandWithCapture, timeoutMs: Math.max(10000, SANDBOXED_RUN_DEFAULT_TIMEOUT_MS) }),
...
runnerWsServer.on("connection", (ws, req) => {
  ...
  ws.on("message", (raw, isBinary) => {
    ...
    if (message.channel === "tts" && message.op === "start") {
      ...
      try {
        let job = operationId ? resolveRunnerWsTtsOperationJob(operationId) : null;
        if (!job) {
